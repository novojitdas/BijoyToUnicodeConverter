...
logo_label.grid(row=0, column=1)  # rc

# Title Window & Icon
root.title("BIJOY TO UNICODE (NOVOJIT)")
root.iconbitmap("icon.pbm")

# converter function (archived codes)


# def callConverter():
# os.system('converterFunction.py')

# new converter function (same file)

def callConverter():
    print("converter function working")
    bijoyText = e1.get()  # text that where inputed into entry field
...
instructions = tk.Label(
    root, text="আপনার বিজয় কিবোর্ডের লেখাটি নিচের বক্সে পেস্ট করুন", font=("Durbar", 40), fg="white", bg="#000000")  # font = ("font-name",font-size)
instructions.grid(row=1, column=1)  # rc


# browse button/convert button (tag button)
#convert_text = tk.StringVar()
# convert_btn = tk.Button(root, textvariable=convert_text, font=(
# "Durbar", 32), bg="#6600cc", fg="#ffff00")
# convert_text.set("Convert!")
# convert_btn.grid(column=1, row=2)  # rc

# entry
e1 = tk.Entry(root, width=100, borderwidth=5,
              justify="left", font=("arial", 16))
e1.grid(column=1, row=4)  # rc

# button of entry
entryButton = tk.Button(root, text="Convert", font=(
    "Durbar", 32), bg="#6600cc", fg="#ffff00", command=callConverter)
entryButton.grid(column=1, row=2)  # use command attribute to use function
...
# canvas span 2 (under button)
# extra height added after button
